...
% output='F:\学习资料\云大作业\医学超声成像处理\filed_II\cyst_phantom\photo';
for j=1:M
...
for i=1:no_lines
  ...
  cmd=['load rf_data/rf_data',num2str(j),'/rf_ln',num2str(i),'.mat'];
  disp(cmd)
  eval(cmd)
...
axis('image')
axis([-20 20 35 90])
% print(gcf,'-djpeg',['photo\',num2str(j),'jpeg']);
priname=['photo\',num2str(j),'.fig'];
saveas(gcf,priname);
end