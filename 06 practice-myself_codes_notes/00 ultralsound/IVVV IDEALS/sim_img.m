...
% Do imaging line by line
        ...
        for i=[1:no_lines]
          ...
          file_name=['rf_data/rf_ln',num2str(i+(j-1)*50),'.mat']

        %   if ~exist(file_name)
        %     
            %  Save a file to reserve the calculation

            cmd=['save rf_data/rf_ln',num2str(i+(j-1)*50),'.mat i'];
            eval(cmd);

            disp(['Now making line ',num2str(i+(j-1)*50)])%ÏÔÊ¾º¯Êý
            ...
            cmd=['save rf_data/rf_ln',num2str(i+(j-1)*50),'.mat rf_data tstart'];
            disp(cmd)
            eval(cmd)
