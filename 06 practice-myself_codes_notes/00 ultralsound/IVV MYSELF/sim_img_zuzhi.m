...
%   return
% else
  load pht_data_zuzhi/pht_data1.mat
%   end
...
for i=[1:no_lines]
  ...
  file_name=['rf_data_zuzhi/rf_ln',num2str(i),'.mat']

%   if ~exist(file_name)
%     
    %  Save a file to reserve the calculation
    
    cmd=['save rf_data_zuzhi/rf_ln',num2str(i),'.mat i'];
    eval(cmd);
    
    disp(['Now making line ',num2str(i)])%ÏÔÊ¾º¯Êý
    ...
    cmd=['save rf_data_zuzhi/rf_ln',num2str(i),'.mat rf_data tstart'];
    disp(cmd)
    eval(cmd)
