...
% Do imaging line by line
            ...
            for i=[1:no_lines]
              ...
              file_name=['rf_data/rf_ln',num2str(i+no_lines*(j-1)),'.mat'];
              %自己把num2str(i+no_lines*(j-1)改成这样
%               if ~exist(file_name)
                %否则就说明存在不运行
              ...
              cmd=['save  rf_data/rf_ln',num2str(i+no_lines*(j-1)),'.mat rf_data tstart']
                disp(cmd)
                eval(cmd);
%               else
%                 cmd=['exit  rf_data/rf_ln',num2str(i+no_lines*(j-1)),'.mat']
%                 disp(cmd)
%                 %否则就说明存在不运行
%           end
end
