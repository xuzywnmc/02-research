...
cmd=['load pht_data/pht_data',num2str(1),'.mat'];
          disp(cmd)
          eval(cmd)
%%%%%%%%%%%%%%%%%关于动态血管模型的说明%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%END%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  vp_1=3/1000;%组织及血管的最大位移
  vr_vessel=zeros(500,20);%500线 20张图所以有20个状态
  %%%%%定义血管内血流的位移%%%%%%%%%%%%%%%%%%%%%%%%%
            for i=1:25
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for j=1:20%二十张图
            for i=1:300
                ...
                %在跑完十张图 每个点回到初始状态
                vr_vessel(i,j)=vp_1*abs(sin(pi*(i/300+j/20)));%定义的血流速度 抛物线函数
            end
    end
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  
 point_up=find(phantom_positions(:,3)<55.5/1000);%先写上边的 找到属于上边的点有哪些(包含血管）
...
%                             phantom_positions(k1,3)=phantom_positions(k1,3)-vr_vessel(k,j); 
%                             
%                             %找到属于每一份内的值所在位置
%                             k2=find(phantom_positions(point_down,1)>=(-25/1000+dx*(k-1)) & phantom_positions(point_down,1)<(-25/1000+dx*k));
%                             %再把这一区域内所有的都加上自己的移动幅度 组织上边是减去幅度z
%                             phantom_positions(k2,3)=phantom_positions(k2,3)+vr_vessel(k,j); 
%                           end
