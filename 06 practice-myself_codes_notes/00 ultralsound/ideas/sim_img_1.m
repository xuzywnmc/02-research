...
%生成几幅图
for j=2:20
           cmd=['load pht_data_1/pht_data',num2str(j),'.mat'];
            eval(cmd);
            %   load pht_data/pht_data1.mat
            %   end
            ...
            %     cmd=['save rf_data/rf_ln',num2str(i+50*(j-1)),'.mat i'];
            %     eval(cmd);

                disp(['Now making line ',num2str(i+50*(j-1))])%显示函数
                ...
                cmd=['save rf_data_1/rf_ln',num2str(i+50*(j-1)),'.mat rf_data tstart'];
                disp(cmd)
                eval(cmd)
