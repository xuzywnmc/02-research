...
%   Load the computer phantom
  ...
  data_name=['pht_data/pht_data',num2str(j),'.mat'];
...
% if ~exist('pht_data.mat')
  ...
  return
    ...
    cmd=['load pht_data/pht_data',num2str(j),'.mat'];
    eval(cmd);
%   load pht_data
  end
...
%   folder_name=['rf_data/rf_ln',num2str(i),'.mat'];
  
  file_name=['rf_data/rf_data',num2str(j),'/rf_ln',num2str(i),'.mat'];

  if ~exist(file_name)
    
    %  Save a file to reserve the calculation
    
    cmd=['save rf_data/rf_data',num2str(j),'/rf_ln',num2str(i),'.mat i'];
    eval(cmd);
    ...
    cmd=['save rf_data/rf_data',num2str(j),'/rf_ln',num2str(i),'.mat rf_data tstart'];
    disp(cmd)
    eval(cmd);
  else
