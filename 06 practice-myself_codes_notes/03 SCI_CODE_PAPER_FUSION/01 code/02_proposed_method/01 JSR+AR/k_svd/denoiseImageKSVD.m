...
reduceDC = 1;
[NN1,NN2] = size(Image);%返回image图像的矩阵的行数与列数
waitBarOn = 1;
if (sigma > 5)
    numIterOfKsvd = 10;
else
...
slidingDis = 1;%
bb = 8;%默认块的大小
maxNumBlocksToTrainOn = 65000;%训练的最大的块数
displayFlag = 1;
...
if(prod([NN1,NN2]-bb+1)> maxNumBlocksToTrainOn)%行数与列数都减去7然后做内积得到图像的块数
    randPermutation =  randperm(prod([NN1,NN2]-bb+1));%随机生成一个1：n中的一个n维的向量
    selectedBlocks = randPermutation(1:maxNumBlocksToTrainOn);%选择selectedBlocks中前1到maxNumBlocksToTrainOn个块进行训练

    blkMatrix = zeros(bb^2,maxNumBlocksToTrainOn);%初始化maxNumBlocksToTrainOn个8×8的0块
    for i = 1:maxNumBlocksToTrainOn
        [row,col] = ind2sub(size(Image)-bb+1,selectedBlocks(i));%image的行数列数都减去7 将第i个selectblock的值赋给其size(Image)-bb+1×size(Image)-bb+1的矩阵相应的位置
        currBlock = Image(row:row+bb-1,col:col+bb-1);%将image中第row行到row+7行，第col列到col+7列赋给currBlock
        blkMatrix(:,i) = currBlock(:);%将currBlock矩阵变为一个向量，在赋给blkMatrix矩阵的第i项
    end
else
    blkMatrix = im2col(Image,[bb,bb],'sliding');%已知道
end

param.K = K;%训练字典集中的原子的个数
...
Pn=ceil(sqrt(K));%表达系数的个数
DCT=zeros(bb,Pn);%生成一个64×Pn的矩阵
for k=0:1:Pn-1,
...
if (reduceDC)
    ...
    blkMatrix = blkMatrix-ones(size(blkMatrix,1),1)*vecOfMeans;%去除blkMatrix矩阵的平均值
end
...
while (prod(floor((size(Image)-bb)/slidingDis)+1)>maxBlocksToConsider)
    slidingDis = slidingDis+1;%滑动距离动态增加 当图像很大时 sliding随着增加 这时块的个数变少
end
