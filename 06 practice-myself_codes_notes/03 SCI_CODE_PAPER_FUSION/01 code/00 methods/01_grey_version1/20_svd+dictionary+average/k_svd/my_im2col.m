...
idxMat = zeros(size(I)-blkSize+1);%生成一个m1×m2的矩阵的0矩阵
% m1为I的行数减去blksize的第一项再加1 列数同理
idxMat([[1:slidingDis:end-1],end],[[1:slidingDis:end-1],end]) = 1; % 将indmat矩阵的行从1每隔slidingDis直到end-1和最后一个元素赋值为1 
% 同理列也是如此 take blocks in distances of 'slidingDix', but always take the first and last one (in each row and column).
...
for i = 1:length(idx)
    ...
    %将image中第rows的第i行到第i行+blkSize(1)-1行 
%     cols的第i列到cols+blkSize(2)-1列赋给currBlock 
