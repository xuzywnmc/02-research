function[Qabf,P,C,QE]=measures(ira,irb,irrt)
Qabf = fusionqp(ira,irb,irrt);
Qabf=(Qabf(:,:,1)+Qabf(:,:,2)+Qabf(:,:,3))/3
% [Qabf]=Qabf_a(ira,irb,irrt)
w=3;[P,C,QE] = fusionQwbnew(ira,irb,irrt,w);

end

function [Qabf] = fusionqp(X1,X2,furt)
...
Qbc=QGbc.*QAbc;
Qabf=sum(sum(Qac.*ga+Qbc.*gb))./sum(sum(ga+gb));
end
