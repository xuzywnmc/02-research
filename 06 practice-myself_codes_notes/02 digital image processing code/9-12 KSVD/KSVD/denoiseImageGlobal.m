...
waitBarOn = 0;  %进度条
C = 1.15;  %最大允许误差
maxBlocksToConsider = 260000;  %最大允许块个数
slidingDis = 1;  %滑动窗滑动距离
bb = 8;  %分块patch边界
...
while (prod(size(Image)-bb+1)>maxBlocksToConsider)  %For vectors, prod(X) is the product of the elements of X.
    slidingDis = slidingDis+1;
end
[blocks,idx] = my_im2col(Image,[bb,bb],slidingDis);  %把图像滑动分块并把分块向量化为列放进block矩阵

if (waitBarOn)
    counterForWaitBar = size(blocks,2);
    h = waitbar(0,'Denoising In Process ...');  %主要是显示进度条
end
...
for jj = 1:10000:size(blocks,2)
    ...
    if (Reduce_DC)
        vecOfMeans = mean(blocks(:,jj:jumpSize));%求每一列的均值
        blocks(:,jj:jumpSize) = blocks(:,jj:jumpSize) - repmat(vecOfMeans,size(blocks,1),1); %减去列均值
    end
    %Coefs = mexOMPerrIterative(blocks(:,jj:jumpSize),D,errT);
    Coefs = OMPerr(D,blocks(:,jj:jumpSize),errT);
    % Sparse coding of a group of signals based on a given 
    % dictionary D and specified number of atoms to use. 
    %the maximal allowed representation error for each siganl.
    %Coefs ： sparse coefficient matrix.
    if (Reduce_DC)
        %block=reshape(D*a+mm+MM,[bb,bb]);
        blocks(:,jj:jumpSize)= D*Coefs + ones(size(blocks,1),1) * vecOfMeans; %如果开始稀疏表示时减掉了列均值，稀疏表示后加上均值
    else
