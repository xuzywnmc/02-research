...
DCT=zeros(bb,Pn);  %8*16 的矩阵
for k=0:1:Pn-1,
    V=cos([0:1:bb-1]'*k*pi/Pn);  %bb*1的列向量  bb=8
    if k>0,
        V=V-mean(V);
    end;
    DCT(:,k+1)=V/norm(V);%norm(V)表示的是欧式距离，归一化V向量
end;
%产生64*256的字典
DCT=kron(DCT,DCT);%http://blog.sina.com.cn/s/blog_7671b3eb0101132y.html  求Kronecker张量积   
%for example : kron(X,Y) = [ X(1,1)*Y  X(1,2)*Y  X(1,3)*Y ; X(2,1)*Y  X(2,2)*Y  X(2,3)*Y ]
%%
%把待系数矩阵分块并向量化放进block矩阵里
...
%%
%利用DCT字典和OMP算法得到的系数系数乘以原图像向量得到稀疏表示后的patch
if (waitBarOn)
...
for jj = 1:10000:size(blocks,2)
    ...
    Coefs = OMPerr(DCT,blocks(:,jj:jumpSize),errT);  %使用OMP方法，DCT字典计算系数系数
