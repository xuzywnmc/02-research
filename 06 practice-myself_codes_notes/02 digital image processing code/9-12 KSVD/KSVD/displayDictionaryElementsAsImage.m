...
% each atom.
%这个程序的功能就是把每一列的原子按照Var从小到大的顺序先把原子排序，然后每个原子按照X*Y 的方块把原子画
%出来，所以画出来的I从左上到右下的patch的Var越来越大，每个patch之间用borderSize宽度的框线隔开


borderSize = 1;%边界宽度，就是蓝色的边界的宽度
columnScanFlag = 1;
strechEachVecFlag = 1;
showImFlag = 1;
...
if (sortVarFlag)%%%%%%%%%%%%%%%%%按照方差从小打大排序
    ...
    D = [D(:,1:sortVarFlag-1),D(:,indices+sortVarFlag-1)];%把D按照Var的大小按列重新排序
    signs = sign(D(1,:));
