...
if (param.preserveDCAtom>0)  %param.preserveDCAtom = 0;  表示是否保留第一个字典列向量
...
if (size(Data,2) < param.K)%  这句的意思是判断原始数据的patch向量化以后的列数是否小于字典的原子数。 K=256    size(Data)=249*249  此句不满足if条件  
    disp('Size of data is smaller than the dictionary size. Trivial solution...');
    Dictionary = Data(:,1:size(Data,2));  %如果小于就把原始数据向量化以后的矩阵作为字典
    return;
...
% elements
if (param.preserveDCAtom)%  这句是判断是否要保留字典的第一个原子不更新。 param.preserveDCAtom = 0;   此句不执行  
...
%上一句的作用：把Dictionary的每一列数据除以该列数据的平方和，从而进行归一化
...
for iterNum = 1:param.numIteration  %param.numIteration = numIterOfKsvd=10
    ...
    if (iterNum>1 & param.displayProgress)%param.displayProgress = 1
        if (param.errorFlag==0)%param.errorFlag = 1;
            ...
            output.numCoef(iterNum-1) = length(find(CoefMatrix))/size(Data,2);%%CoefMatrix中所有非0元素的长度/DATA的列数=平均每列非零系数的个数
